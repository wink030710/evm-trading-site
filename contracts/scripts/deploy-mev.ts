...
function compileMev(): { abi: ethers.InterfaceAbi; bytecode: string } {
  const sourcePath = path.resolve(__dirname, "..", "contracts", "Mev.sol");
  const source = fs.readFileSync(sourcePath, "utf8");
  const input = {
    language: "Solidity" as const,
    sources: { "Mev.sol": { content: source } },
    settings: {
  ...
  const contract = output.contracts?.["Mev.sol"]?.Mev;
  if (!contract) throw new Error("Mev not found in compiler output");
...
async function main(): Promise<void> {
  ...
  const { abi, bytecode } = compileMev();
  const factory = new ethers.ContractFactory(abi, bytecode, wallet);
  console.log("Deploying Mev...");
  ...
  const outFile = path.resolve(outDir, "bittensor-Mev.json");
